<template>
  <default-field :field="field" :errors="errors">
    <template #field>
      ...
      <modal
        :show="true"
        v-if="processingResource"
        role="dialog"
        @modal-close="cancelProcessing"
      >
        <form
          autocomplete="off"
...
export default {
  ...
  methods: {
    ...
    isDuplicate(tags, tag) {
      return (
        !this.field.duplicate && tags.map((tag) => tag.id).indexOf(tag.id) >= 0
      );
    },
    ...
    async attachTheResource() {
      if (await this.validatePivotFields(this.processingResource)) {
        ...
        index =
          typeof index == "number" ? index : this.attachedResources.length - 1;
        this.pivots[index] = this.attachmentFormData;

        this.attachedResources[index] = _.tap(
          this.attachedResources[index],
          (tag) => {
            tag.attached = false;
    ...
    getAvailableResources() {
      Nova.request()
        .get(
          `/nova-api/armincms/${this.resourceName}/nonattachable/${this.field.resourceName}`,
          {
            params: {
              search: this.search,
              field: this.field.attribute,
    ...
    getAttachedResources() {
      Nova.request()
        .get(
          `/nova-api/armincms/${this.resourceName}/nonattached/${this.field.resourceName}`,
          {
            params: {
              field: this.field.attribute,
              resourceId: this.resourceId,
