...
{  
    ...
    protected function fillAttributeFromRequest(NovaRequest $request, $requestAttribute, $model, $attribute)
    {
        if ($request->exists($requestAttribute)) {
            ...
            $model::saved(function($model) use ($value, $request) {
                ...
                if(! $this->duplicate) { 
                    $attaching = $this->removeDuplicateAttachments($model, $attaching)
                                        ...
                                        ->map(function($attachment) { 
                                            return $this->removeHelperData($attachment);
                                        })->all();
                    ...
                    $attaching->each(function($attachment) use ($relationship) { 
                        $relationship->attach(
                            $attachment['id'], $this->removeHelperData($attachment)
                        );
                    }); 
                }  
            }); 
    ...
    /**
     * Remove data that append for vuejs
     * 
     * @param  array $attachment 
     * @return array             
     */
    protected function removeHelperData($attachment)
    {
        return collect($attachment)->reject(function($value, $key) {
            return in_array($key, ['id', 'attached', 'pivotId']);
        })->all(); 
    }
